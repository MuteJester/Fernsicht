...
export function showBroadcasterView() {
    $("landing").classList.add("hidden");
    $("header").classList.remove("hidden");
    $("broadcaster-view").classList.remove("hidden");
    $("viewer-view").classList.add("hidden");
}
export function showViewerView() {
    $("landing").classList.add("hidden");
    $("header").classList.remove("hidden");
    $("broadcaster-view").classList.add("hidden");
...
export function createProgressBar(taskId, label) {
...
export function updateProgressBar(taskId, value) {
    const bar = activeBars.get(taskId);
    if (!bar)
        return;
    const percent = Math.min(100, Math.round(value * 100));
    const fill = bar.querySelector(".progress-fill");
    const pct = bar.querySelector(".progress-pct");
    fill.style.width = `${percent}%`;
    pct.textContent = `${percent}%`;
}
export function completeProgressBar(taskId) {
    const bar = activeBars.get(taskId);
    if (!bar)
        return;
    const fill = bar.querySelector(".progress-fill");
...
// --- Broadcaster: log ---
export function appendBroadcasterLog(message) {
